#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano correcoes-implantador` em 2026-09-21 16:56
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo correcoes-implantador
set -u
set -o pipefail

RUN_ID='unicoImplantacao-correcoes-implantador-mubhmgdj'
FEATURE='correcoes-implantador'
BASE_BRANCH='spec/correcoes-implantador'
ENGINE='C:\Users\Comercial\.agents\skills\onp-spec-driven\scripts\onp-spec.mjs'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/correcoes-implantador/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/unicoImplantacao-correcoes-implantador-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/unicoImplantacao-correcoes-implantador"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/correcoes-implantador/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/correcoes-implantador-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-001' 'Você executa UMA tarefa da feature "correcoes-implantador" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/correcoes-implantador/spec.md, .spec/features/correcoes-implantador/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-001 — "Criar testes de regressão para processadores"
  critérios/refs: AC-001 (Distribuição automática desativada), AC-002 (Horário e mensagem fora do horário), AC-003 (Mensagem de encerramento), AC-005 (Reuso sem duplicação), AC-009 (Nome curto aceito), AC-012 (Template de URA de referência)
  arquivos permitidos (e seus testes): server/src/jobs/processors/__manual-test.ts, server/package.json
  mensagem de commit: "T-001 correcoes-implantador: Criar testes de regressão para processadores"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/correcoes-implantador-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/correcoes-implantador-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-002' 'Você executa UMA tarefa da feature "correcoes-implantador" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/correcoes-implantador/spec.md, .spec/features/correcoes-implantador/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-002 — "Corrigir defaults e contrato de filas"
  critérios/refs: AC-001 (Distribuição automática desativada), AC-002 (Horário e mensagem fora do horário), AC-003 (Mensagem de encerramento), AC-004 (Motivo de encerramento padrão)
  arquivos permitidos (e seus testes): features/onboarding/queue-defaults.ts, features/onboarding/types.ts, features/onboarding/components/QueueFormDialog.tsx, server/src/jobs/processors/configure-queues.ts, server/src/integrations/atender-bem/business-hours.ts
  mensagem de commit: "T-002 correcoes-implantador: Corrigir defaults e contrato de filas"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium &&
    rodar_tarefa 'faixa-2' 'T-007' 'Você executa UMA tarefa da feature "correcoes-implantador" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/correcoes-implantador/spec.md, .spec/features/correcoes-implantador/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-007 — "Refinar descoberta das opções de fila"
  critérios/refs: AC-011 (Hierarquia visual das opções)
  arquivos permitidos (e seus testes): features/onboarding/components/QueueFormDialog.tsx
  mensagem de commit: "T-007 correcoes-implantador: Refinar descoberta das opções de fila"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/correcoes-implantador-faixa-2' "$WT" "$st" || return 1
  marcar_concluidas T-002 T-007
  return 0
}
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/correcoes-implantador-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-003' 'Você executa UMA tarefa da feature "correcoes-implantador" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/correcoes-implantador/spec.md, .spec/features/correcoes-implantador/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-003 — "Reabrir onboarding com auditoria"
  critérios/refs: AC-006 (Consulta após execução), AC-007 (Reabertura segura)
  arquivos permitidos (e seus testes): server/src/modules/onboarding/onboarding.service.ts, server/src/modules/implantations/implantation.controller.ts, server/src/modules/implantations/implantation.routes.ts, server/src/modules/audit-logs/audit-log.constants.ts, features/implantations/api.ts, features/implantations/components
  mensagem de commit: "T-003 correcoes-implantador: Reabrir onboarding com auditoria"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/correcoes-implantador-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/correcoes-implantador-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-004' 'Você executa UMA tarefa da feature "correcoes-implantador" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/correcoes-implantador/spec.md, .spec/features/correcoes-implantador/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-004 — "Reutilizar administrador principal"
  critérios/refs: AC-005 (Reuso sem duplicação)
  arquivos permitidos (e seus testes): server/src/jobs/processors/create-users.ts, server/src/integrations/atender-bem/users.ts
  mensagem de commit: "T-004 correcoes-implantador: Reutilizar administrador principal"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium &&
    rodar_tarefa 'faixa-4' 'T-005' 'Você executa UMA tarefa da feature "correcoes-implantador" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/correcoes-implantador/spec.md, .spec/features/correcoes-implantador/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-005 — "Melhorar seleção e nomes de usuários"
  critérios/refs: AC-008 (Seleção de todas as filas), AC-009 (Nome curto aceito)
  arquivos permitidos (e seus testes): features/onboarding/components/UserFormDialog.tsx, server/src/jobs/processors/create-users.ts
  mensagem de commit: "T-005 correcoes-implantador: Melhorar seleção e nomes de usuários"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/correcoes-implantador-faixa-4' "$WT" "$st" || return 1
  marcar_concluidas T-004 T-005
  return 0
}
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/correcoes-implantador-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-006' 'Você executa UMA tarefa da feature "correcoes-implantador" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/correcoes-implantador/spec.md, .spec/features/correcoes-implantador/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-006 — "Validar respostas rápidas no onboarding"
  critérios/refs: AC-010 (Validação antes do envio)
  arquivos permitidos (e seus testes): features/onboarding/steps/QuickRepliesStep.tsx, features/onboarding/OnboardingWizard.tsx, server/src/modules/onboarding/onboarding.schema.ts
  mensagem de commit: "T-006 correcoes-implantador: Validar respostas rápidas no onboarding"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/correcoes-implantador-faixa-5' "$WT" "$st" || return 1
...
executar_faixa_6() {
  ...
  preparar_worktree 'faixa-6' 'spec/correcoes-implantador-faixa-6' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-6' --estado executando --tentativa "$(tentativa 'faixa-6')"
  : > "$LOG_DIR/faixa-6.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-6' 'T-008' 'Você executa UMA tarefa da feature "correcoes-implantador" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/correcoes-implantador/spec.md, .spec/features/correcoes-implantador/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-008 — "Substituir template de URA"
  critérios/refs: AC-012 (Template de URA de referência)
  arquivos permitidos (e seus testes): server/src/integrations/atender-bem/defaults/ivr-boas-vindas-basico.json, server/src/integrations/atender-bem/defaults/index.ts, server/src/jobs/processors/configure-ivr.ts
  mensagem de commit: "T-008 correcoes-implantador: Substituir template de URA"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-6' 'spec/correcoes-implantador-faixa-6' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/correcoes-implantador/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-2  onda 1  T-002, T-007"
  echo "  faixa-3  onda 1  T-003"
  echo "  faixa-4  onda 2  T-004, T-005"
